...
class QueryTokenizer
  def tokenize(str)
    ...
    until s.eos?
      ...
      elsif s.scan(/(is|not)\s*[:=]\s*(vanilla|spell|permanent|multipart|promo|primary|secondary|front|back|commander|reprint|canon|tale|shockfetch|mirrorland|monofetch|plagueland|tormentland|cycleland|handland|investigateland|checkland|tricheck|tangoland|unique|draft|historic|staple|playable|perfect|scuttleback|brawler|storied|modal|hugo|searle|mable|marisa)\b/i)
        tokens << [:not] if s[1].downcase == 'not'
        cond = s[2].capitalize
        cond = 'Timeshifted' if cond == 'Colorshifted'
        klass = Kernel.const_get("ConditionIs#{cond}")
        tokens << [:test, klass.new]
      elsif s.scan(/(is|not|layout)\s*[:=]\s*(normal|leveler|vanguard|dfc|double-faced|token|split|flip|plane|scheme|phenomenon|meld|aftermath|adventure|saga)\b/i)
        tokens << [:not] if s[1].downcase == 'not'
