...
class ConditionSetType < Condition
    ...
    db.sets.each do |_set_code, set|
      ...
      if set.code == 'cm1'
        if type_list.include?('fixed')
          sets << set
        elsif type_list.include?('commander') && !type_list.include?('deck')
          sets << set
        end
        next
      end
      ...
      if (%w[cp1 cp2 cp3].include?(set.code) && type_list.include?('deck')) ||
         (%w[p3k po po2 st].include?(set.code) && type_list.include?('booster')) ||
         (%w[st2k w16 itp].include?(set.code) && type_list.include?('fixed'))
        sets << set
      end
      ...
      sets << set if type_list.include?(set.type)
    end
    sets
  end
  ...
  def get_type_list(set_type)
    case set_type
    when 'multi'
      %w[archenemy commander conspiracy planechase vanguard multi]
    when 'booster'
      %w[expansion conspiracy reprint core masters un booster]
    when 'fixed'
