...
describe 'Banlist' do
  ...
  def assert_commander_banlist_changes(date, *changes)
    assert_banlist_changes(
      date,
      *changes.each_slice(2).flat_map do |s, c|
        ["commander #{s}", c]
      end
    )
  end
  ...
  it 'vintage_banned_means_commander_banned' do
    BanList.all_change_dates.each do |date|
      ...
      vintage_banned = vintage_banlist.select { |_c, s| s == 'banned' }.map(&:first)
      commander_banned = commander_banlist.select { |_c, s| s == 'banned' }.map(&:first)
