...
describe 'Formats - Block Constructed' do
  ...
  describe 'Block legal sets' do
    ...
    let(:block_formats) { Format.formats_index.select { |k, _v| k =~ /block\z/ }.values.uniq }
    let(:expansion_sets) { db.sets.values.select { |s| s.type == 'expansion' } }
    let(:expected) do
      expansion_sets.select do |s|
        s.release_date >= start_date and s.release_date <= end_date and s.code != 'hl'
      end.map(&:code).to_set
    end
  ...
  it 'ice_age' do
    ...
    assert_block_composition 'ice age block', 'ai', %w[ia ai],
                             'Amulet of Quoz' => 'banned'
    assert_block_composition 'ice age block', 'cs', %w[ia ai cs],
  ...
  it 'tempest' do
    ...
    assert_block_composition 'tempest block', 'sh', %w[tp sh],
                             'Cursed Scroll' => 'banned'
    assert_block_composition 'tempest block', 'ex', %w[tp sh ex],
                             'Cursed Scroll' => 'banned'
  end

  it 'urza' do
    assert_block_composition 'urza block', 'us', ['us']
    assert_block_composition 'urza block', 'ul', %w[us ul]
    assert_block_composition 'urza block', 'ud', %w[us ul ud],
                             'Memory Jar' => 'banned',
                             'Time Spiral' => 'banned',
                             'Windfall' => 'banned'
    assert_block_composition 'urza block', nil, %w[us ul ud],
  ...
  it 'masques' do
    ...
    assert_block_composition 'masques block', nil, %w[mm ne pr],
  ...
  it 'mirrodin' do
    ...
    assert_block_composition 'mirrodin block', nil, %w[mi ds 5dn],
                             'Aether Vial' => 'banned',
                             'Ancient Den' => 'banned',
  ...
  it 'time_spiral' do
    ...
    assert_block_composition 'time spiral block', 'ts', %w[ts tsts]
    assert_block_composition 'time spiral block', 'tsts', %w[ts tsts]
    assert_block_composition 'time spiral block', 'pc', %w[ts tsts pc]
    assert_block_composition 'time spiral block', 'fut', %w[ts tsts pc fut]
  end
  ...
  it 'innistrad' do
    ...
    assert_block_composition 'innistrad block', 'dka', %w[isd dka]
    assert_block_composition 'innistrad block', 'avr', %w[isd dka avr],
  ...
  it 'unsets' do
    ...
    assert_block_composition 'unsets', 'uh', %w[ug uh],
                             'Once More with Feeling' => 'restricted'
    assert_block_composition 'unsets', 'ust', %w[ug uh ust],
                             'Once More with Feeling' => 'restricted'
  end
end
