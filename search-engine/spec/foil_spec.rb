  ...
  def assert_foiling_partial_precon(cards)
    cards.each do |card|
      ...
      if card.set.cards_in_precons
        ...
        elsif has_nonfoil && !has_foil
          card.foiling.should eq('nonfoil'), "#{card} should be nonfoil"
        elsif has_nonfoil && has_foil
          card.foiling.should eq('totally broken'),
                              "#{card} is marked as both foil and nonfoil, that is wrong for precon cards generally"
        else
  ...
  it do
    db.sets.each do |_set_code, set|
      ...
      when 'st2k'
        ...
        sampler, regular = rest.partition do |c|
          ['Armored Pegasus', 'Python', 'Spined Wurm', 'Stone Rain'].include?(c.name)
        end
      ...
      when 'ptc'
        ...
        assert_foiling(cards[3..], 'foilonly')
      when 'akh'
        booster_cards, extra_cards = set.printings.partition(&:in_boosters?)
        assert_foiling(booster_cards, 'both')
