...
describe 'Unsets' do
  ...
  it 'name search and weird card names' do
    db.cards.each_value do |card|
      ...
      assert_search_include card.name.to_s, card.name
      assert_search_include card.name.downcase.to_s, card.name
      assert_search_include card.name.upcase.to_s, card.name
    end
  end
  ...
  it '//' do
    assert_search_results '//', 'Who', 'What', 'When', 'Where', 'Why', 'B.F.M. (Big Furry Monster)',
                          'B.F.M. (Big Furry Monster, Right Side)', 'Naughty', 'Nice'
  ...
  it 'display_power / display_toughness' do
    validate = proc do |_card, value, display_value|
      case value
      when nil
      ...
      when Float
        raise unless value.to_s =~ /\A(\d+)\.5\z/

        display_value.should eq("#{Regexp.last_match(1)}½")
      when String
        display_value.should eq(value)
      else
    ...
    db.cards.each_value do |card|
      if card.augment
        ...
        validate.call(card, card.power, card.display_power)
        validate.call(card, card.toughness, card.display_toughness)
      end
    end
  end
end
